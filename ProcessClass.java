...
import java.io.*;
import java.net.*; // java.io. Provides for system input and output through data streams, serialization and the file system.
import java.util.Iterator;  //Getting an Iterator. The iterator() method can be used to get an Iterator for any collection.
import java.util.LinkedHashMap; // Hash table and Linked list implementation of the Map interface, with predictable iteration order.
import java.util.Map.Entry; // A map entry (key-value pair). The Map.entrySet method returns a collection-view of the map, whose elements are of this class.
import java.util.Random; // Creates a new random number generator using a single long seed.
...
public class ProcessClass implements Runnable {
    ...
    private DatagramSocket socket; //bunch of information sent in a single logical packet.
    private int portNo;
    ...
    public ProcessClass() throws IOException {
        ...
        counter = new Random().nextInt(20); // The nextInt() is used to get the next random integer value from this random number generator's sequence.

    }

    public ProcessClass(int portNo) {  // class to catch the error if some ambiguity occurs in prg.
        try {
            hostAddress = InetAddress.getLocalHost(); 
            this.portNo = portNo;
    ...
    public void initCoordinator() {
        try {
            System.out.print("Enter the number of additional processes for communicating(min. : 2 ): ");
            int max_process = 0;
    ...
    // Sending the time when each process has been created.
...
// Class for creation of Average 
    ...
    private void startBerkleyAsSlave() {
        try {
            ...
            System.out.println("Sorry, could not recieve file : " + ex);
        }
    }
